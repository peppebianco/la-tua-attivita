      ...
      <div className="mt-14 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
        {SERVICES.map((service, index) => (
          <Reveal key={service.title} delay={index * 100}>
            <div className="group flex h-full flex-col rounded-2xl border border-slate-100 p-6 shadow-sm transition hover:-translate-y-2 hover:border-brand-green-100 hover:shadow-xl">
              <span
                className={`flex h-12 w-12 items-center justify-center rounded-xl transition-transform duration-300 group-hover:-rotate-6 group-hover:scale-110 ${service.color}`}
              >
